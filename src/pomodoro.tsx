...
import { BearIcon } from './components/bear-icon.tsx';
...
export const Pomodoro: React.FunctionComponent = () => {
  ...
  return (
    <div className="w-full md:w-72 bg-customCard p-4 rounded-lg shadow-xl">
      <h2 className="text-lg font-bold mb-4 text-gray-900">Pomodoro</h2>
      <div className="flex flex-col items-center">
        <div className="text-5xl font-bold mb-6 text-gray-900 tabular-nums">
          {isBreak ? formatTime(breakTime) : formatTime(time)}
        </div>
        
        <div className="grid grid-cols-2 gap-4 mb-4 w-full">
          <div>
            <label className="block text-sm text-gray-700 mb-1">Trabajo (min)</label>
            <input
              type="number"
              value={inputTime}
              onChange={handleTimeChange}
              className="p-2 rounded-md border w-full text-center bg-white text-gray-900 border-customCard focus:outline-none focus:border-blue-500"
              min="1"
              disabled={isActive}
            />
          </div>
          <div>
            <label className="block text-sm text-gray-700 mb-1">Descanso (min)</label>
            <input
              type="number"
